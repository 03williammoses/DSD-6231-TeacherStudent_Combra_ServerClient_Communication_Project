...
public class MontrealServer extends DCMSInterfacePOA{
	...
	private int port;
	public static Logger logger = Logger.getLogger("Montreal");
	...
	protected MontrealServer(String location) throws RemoteException {
		teacherStudentRecords = new HashMap<>();
		List<Record> ar = new ArrayList<>();
		ar.add(new TeacherRecord("MTL1234", "A", "B", "123 C", "1234567890", "CS", "MTL"));
		ar.add(new TeacherRecord("MTL1234", "A", "B", "123 C", "1234567890", "CS", "MTL"));
		ar.add(new TeacherRecord("MTL1234", "A", "B", "123 C", "1234567890", "CS", "MTL"));
		teacherStudentRecords.put("B", ar);
	...
	public void serverConnection(int port, MontrealServer montrealServ) throws RemoteException, AlreadyBoundException {
		addLog("src/main/resources/logs/server/Montreal.txt", "Montreal");
	...
	synchronized public String editRecord(String managerID, String recordID, String fieldName, String newValue) {
		...
		if (recordID.startsWith("SR")) {
			if (existsInList(STUDENT_INFO, fieldName)) {
				...
				logger.info("Student Record doesn't exists or it is not allowed to modify the given field.");
				return "Student Record doesn't exists or it is not allowed to modify the given field.";
			}
			...
			if (existsInList(TEACHER_INFO, fieldName)) {
				...
				logger.info("Teacher Record doesn't exists or it is not allowed to modify the given field.");
				return "Teacher Record doesn't exists or it is not allowed to modify the given field.";
			}
		}else {
	...
	private String updateTeacherRecord(String recordID, String fieldName, String newValue) {
		...
		for (Entry<String, List<Record>> entry : teacherStudentRecords.entrySet()) {
			for (Object object : entry.getValue()) {
				if (object instanceof TeacherRecord && ((TeacherRecord) object).getRecordID().equalsIgnoreCase(recordID)) {
...
//	}
	...
	synchronized public String getRecordCounts() {
		...
		server1 += "MTL : " + mapSize;
		server2 += DatafromOtherServers(8881);
		server3 += DatafromOtherServers(8882);
	...
	public String transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		if(remoteCenterServerName.equalsIgnoreCase("LVL")) {
			...
			record = StringFromOtherServers(8882, recordID).split(",");
		}
